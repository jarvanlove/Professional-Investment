...
- **黄金测试（核心）**：PDF 第 13 章算例 A/B/C 转为 pytest 参数化用例，输出金额必须与 PDF 表格一致（算例 A：财通 ≤663 元、长盛 ≤477 元、广发不动、摩根 ≤1,068 元、合计约 2,208 元 → 口径披露：实际执行合计 1,544 元 = 长盛 477 + 摩根 1,067；财通因每周 2 单元预算严格执行顺延，广发入买入死区 300元/1.5%总资产——PDF 8.4 的 2 单元周预算为规范性约束、优先于算例演示算术，3pp 死区仅适用于卖出）
