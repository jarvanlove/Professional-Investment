...
2. 打开 http://localhost:3010/signals —— 点"1. 更新净值"（akshare 抓取四只基金净值），再点"2. 计算信号"。
...
先在「设置」页选择 LLM 供应商（DeepSeek / Kimi / MiniMax / Qwen / GLM，含通用与 Coding 套餐），系统会自动填充对应 Base URL；
填写 API Key 后，可点击"获取模型列表"实时拉取供应商可用模型。
AI 只解释报告，不参与计算。

已知口径：
- 027521（广发）使用 025343 作为信号代理，需累计 ≥61 个净值点；在此之前信号计算返回 422 数据不足（by design）。
